...
def get_neighbours(node):
	x, y, z, w = unstringify(node)

	for x_delta in range(-1, 2):
		for y_delta in range(-1, 2):
			for z_delta in range(-1, 2):
				for w_delta in range(-1, 2):
					if x_delta != 0 or y_delta != 0 or z_delta != 0 or w_delta != 0:
						yield stringify(x + x_delta, y + y_delta, z + z_delta, w + w_delta)
...
if __name__ == '__main__':
	...
	print('Day 17 : Conway Cubes - part 2')
